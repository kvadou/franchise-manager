...
async function getCampaignData(dateRange: { start: Date; end: Date }) {
  // Get spend data from LeadSource + CampaignSpend for the period
  const startMonth = dateRange.start.getMonth() + 1;
  const startYear = dateRange.start.getFullYear();
  const endMonth = dateRange.end.getMonth() + 1;
  const endYear = dateRange.end.getFullYear();

  const leadSourcesWithSpend = await db.leadSource.findMany({
    where: { isActive: true },
    include: {
      spends: {
        where: {
  ...
  // Build a spend lookup by utm source/medium
  const spendBySource = new Map<string, number>();
  let totalSpend = 0;
  for (const ls of leadSourcesWithSpend) {
    const spend = ls.spends.reduce((sum, s) => sum + Number(s.amount), 0);
    if (spend > 0) {
      const key = `${ls.utmSource || ""}|${ls.utmMedium || ""}`;
      spendBySource.set(key, (spendBySource.get(key) || 0) + spend);
      totalSpend += spend;
    }
  }
  ...
  const campaigns = Array.from(campaignMap.values())
    .map((c) => {
      // Match spend by source/medium
      const spendKey = `${c.source}|${c.medium || ""}`;
      const sourceOnlyKey = `${c.source}|`;
      const spend = spendBySource.get(spendKey) || spendBySource.get(sourceOnlyKey) || 0;
      const cpl = c.conversions > 0 && spend > 0 ? spend / c.conversions : 0;
  ...
  const totalCpl = totals.conversions > 0 && totalSpend > 0
    ? totalSpend / totals.conversions
    : 0;

  return { campaigns, contentVariants, totals, totalSpend, totalCpl };
}
...
export default async function CampaignsPage({
  ...
  const { campaigns, totals, totalSpend, totalCpl } = await getCampaignData(dateRange);
          ...
          <CardContent className="py-4">
            ...
            {totalSpend > 0 ? (
              ...
              <>
                ...
                <div className="text-xs text-gray-400">
                  No spend data &mdash;{" "}
                  <a href="/admin/marketing/lead-sources" className="text-brand-purple hover:underline">
                    Add via Lead Sources
                  </a>
                </div>
              </>
            )}
                ...
                <tbody className="divide-y divide-gray-100">
                  {campaigns.map((campaign, i) => (
                    <tr key={i} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-right">
                        {campaign.cpl > 0 ? (
                          <span className="font-medium text-brand-navy">${campaign.cpl.toFixed(2)}</span>
                        ) : (
